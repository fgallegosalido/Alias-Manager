...
# This script highly rely that in your .bashrc file there's a block of bash code
...
#   fi
#
# What this block does is execute the script called ".bash_aliases" (in fact
# that script is where all your "permanent" aliases are going to be defined).
#
# If that block doesn't exists in your .bashrc file, then this script is just
...
# instead or relying on the invocation of .bash_aliases, but it's better not to
...
# Just check if th file .bash_aliases exists in the home directory,
# and if not, initialize it with an alias to the path of this script
...
if [ $# -eq 0 ]; then
  ...
  if [ "$1" == "-n" ] || [ "$1" == "--new" ]; then
    ...
    sed 's/alias //g' $HOME/.bash_aliases
